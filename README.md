...
```
This already indicates two possible selection criteria for disappearing tracks.
...
In the following, we will use a basic track categorization. Tracks which are reconstructed in the pixel tracker are classified as pixel-only tracks, while tracks in both the pixel and strips tracker are classified as pixel+strips tracks. The categorization is as follows:
...
Boosted decision trees are a form of a multivariate analysis. An introduction to boosted decision trees is given [here](http://www.if.ufrj.br/~helder/20070706_hh_bdt.pdf). We will train two separate BDTs, one for each track category, using TMVA ([Toolkit for Multivariate Analysis](https://root.cern/tmva)) included in ROOT.
...
TMVA stores the output by default in "output.root" and a folder containing the weights of the BDT along with a C helper class to apply the weights to a given event. One way to compare the different TMVA outputs is to overlay the different ROC curves.
